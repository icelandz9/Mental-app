...
**การทำงาน:** เมื่อกดปุ่มสมัครสมาชิก ระบบจะ (1) ตรวจว่ารหัสผ่านกับช่องยืนยันตรงกันหรือไม่ ถ้าไม่ตรงจะแจ้งเตือนแล้วหยุด (2) สร้างบัญชีใหม่ด้วย `createUserWithEmailAndPassword()` บน Firebase Authentication ซึ่งจะได้ `uid` (รหัสประจำตัวผู้ใช้) กลับมา (3) บันทึกชื่อที่กรอกลง Firestore ที่เอกสาร `users/{uid}` โดยใช้ `merge: true` เพื่อไม่ให้ทับข้อมูลอื่นที่กรอกทีหลัง (4) ถ้าเกิดข้อผิดพลาด เช่น อีเมลซ้ำหรือรหัสผ่านสั้นเกินไป จะถูกดักจับด้วย `FirebaseAuthException` แล้วแสดงข้อความเตือน
